...
- `-s`/`--summary` is gone. It did nothing (per-file summaries for changed files print by default), and every wrapper script that passed it was updated in the same commit. The nzbget scripts are the loose end: `#SCRIPT_ARGS=-s -q` is only the *default* — nzbget persists the user's value in `nzbget.conf`, so an existing install keeps sending `-s` until the setting is edited in the nzbget UI. That run exits 2 from argparse, and `nzbget-postprocess.sh` exits 93 regardless, so nzbget reports success while nothing was processed.
