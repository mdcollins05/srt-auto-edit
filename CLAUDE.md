...
- The never-fired *count* prints at `V_NORMAL`, but the *names* need `V_CHANGES`. That list is as long as the config rather than as long as the run, so at default verbosity a single-file post-process run against a 70-rule config printed 25 lines of rule names under a one-line summary. Both branches are covered by the console baselines: the `-v`/`-vv` cases take the list, `default-verbosity` and `missing-path` take the hint line. (`no-arguments` doesn't — it exits before the rollup.)
